...
const CalendarDay: React.FC = (): React.ReactElement => {
  ...
  const mappedEvents: Array<Event & { id?: string }> = useMemo(() => {
    if (state.loading)
      return getDefaultDates(selectedDate, selectedDate, "day");
  ...
  const mappedBackgroundEvents: Array<Event & { id?: string }> = useMemo(() => {
    ...
    return externalCalendarEvents
      .filter(
        ({ endTime, busy }) =>
          dayjs(endTime).format("YYYY-MM-DD") ===
            dayjs(selectedDate).format("YYYY-MM-DD") && busy
      )
      .map((event) => ({
        id: event?.id,
        title: JSON.stringify({
  ...
  const { handlers, scrollingUpOrDown, tapped, refPassthrough } =
    UseSwipeGesture({
      parentRef: pageRef,
    ...
    (value: SlotInfo) => {
      if (value && tapped && !scrollingUpOrDown) {
        if ("start" in value && "end" in value) {
          const eventExist = [...mappedEvents, ...mappedBackgroundEvents].find(
            (event) => {
              if (
  ...
  useEffect(() => {
    if (location.pathname === CALENDAR_DAY && !isCreateAppointmentOpen) {
      if (state.loading) {
        dispatch(
          setLoading({ loading: true, message: `${t("loading_appointments")}` })
        );
      }
