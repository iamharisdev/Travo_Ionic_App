...
export const prevMonth = (date: string) =>
  dayjs(date).startOf('month').subtract(1, 'month').toDate();

export const getDefaultDates = (start: string, end: string, type: 'day' | 'week' | 'month') => {
  const arr: Array<Date> = [];

  for (
    const dt = new Date(dayjs(start).startOf(type).toDate());
    dt <= new Date(dayjs(end).endOf(type).toDate());
