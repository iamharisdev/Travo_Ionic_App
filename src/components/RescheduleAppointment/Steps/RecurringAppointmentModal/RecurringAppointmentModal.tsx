...
const RecurringAppointmentModal: React.FC<RecurringAppointmentModalInterface> = ({recurringFrequency, recurringCount, startTime, providerId, practiceId, event}) => {
...
console.log("invoiceDataId : ", event)
  ...
  const handleOpenModal=async()=>{
    ...
    const response = await dispatch(getEventsAction({
    ...
    console.log("response : ", response?.payload?.events)
  }
  const appointmentConfirmedStatus= appointmentStatus.filter(event => event?.status === "Confirmed")
      ...
      <IonItem lines="none" style={{ alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', marginRight: '3px' }}>
          <IonIcon
            ...
            style={{ color: 'var(--ion-trova-medium-gray)', fontSize: '18px', marginLeft: '5px', marginTop: "7px" }}
          />
          <IonIcon
            icon={returnUpForwardOutline}
            style={{ color: 'var(--ion-trova-medium-gray)', fontSize: '18px', marginLeft: '-32px', marginBottom: "7px" }}
          />
        </div>
            ...
            <IonItem lines="none">
              <IonText className={`${CSSprefix}-title`}>
                ...
                <p className={`${CSSprefix}-description`}>
                  <IonItem lines="none" className={`${CSSprefix}-input ${CSSprefix}-select-input-field`}>
                    <IonLabel position="stacked" className={`${CSSprefix}-lables`}>{t("recurring_appointment_repeats_on")}</IonLabel>
                    <IonSelect
                  ...
                  <IonItem lines="none" className={`${CSSprefix}-input`}>
                    <IonLabel position="stacked" className={`${CSSprefix}-lables`}>{t("Ends after (occurrences number)")}</IonLabel>
                    <IonInput
