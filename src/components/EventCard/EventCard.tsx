...
const EventCard: React.FC<any> = ({
  ...
  const eventProps: {
    ...
    redirect?: boolean;
  } = useMemo(() => JSON.parse(rest.event.title), [rest.event.title]);
  ...
  const content = useMemo(() => {
    ...
    if (index === 4) {
      return (
        <p className="more">
          {lang == 'en' ? `${eventsLeft} ${t('more_events')}` : `${t('more_events')} ${eventsLeft}`}
        </p>
      );
    }
        ...
        >
          ...
          {showExtraInformation && (
            <div className={`${CSSprefix}-patient-container`}>
              ...
              {eventProps?.isMeetingEvent && (
                <IonText className={`${CSSprefix}-patient ion-no-margin`}>{`${dayjs(
                  eventProps.start
                ).format('HH:mm A')} - ${dayjs(eventProps.end).format('HH:mm A')}`}</IonText>
              )}
            </div>
          )}
        </div>
  ...
  return (
    <>
      ...
      {loading && (
        <IonSkeletonText
          animated={true}
          style={{ ...children.props.style, width: '100%', height: '30px', borderRadius: '8px' }}
        />
      )}
    </>
  );
