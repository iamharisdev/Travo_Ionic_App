...
import {
  ...
  rescheduleAppointmentWithOutRecurring,
  ServicesResponse,
} from '../api/services/scheduling';
import {
...
export const cancelAppointmentAction = createAsyncThunk(
  ...
  async ({
    ...
    try {
      await cancelAppointment(practiceId, providerId, appointmentId, endPoint, payload);
...
export const rescheduleAppointmentAction = createAsyncThunk(
  ...
  async ({
    ...
    try {
      console.log('payload?.recurring:=>  ', payload);
      if (payload?.recurring) {
        await rescheduleAppointment(practiceId, providerId, appointmentId, payload);
      } else {
        await rescheduleAppointmentWithOutRecurring(practiceId, providerId, appointmentId, payload);
      }
      return true;
    } catch (error: any) {
...
export const editAppointmentRecurringAction = createAsyncThunk(
  'scheduling/editAppointmentRecurring',
  async ({
    practiceId,
    providerId,
    appointmentId,
    ...
    try {
      await editAppointmentsRecurring(practiceId, providerId, appointmentId, payload);
...
const schedulingSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(
        getMicrosoftEventsAction.fulfilled,
        (state, action: PayloadAction<Array<IAppointment>>) => {
      ...
      .addCase(
        getGoogleEventsAction.fulfilled,
        (state, action: PayloadAction<Array<IAppointment>>) => {
      ...
      .addCase(
        editAppointmentAction.fulfilled,
        (
          state,
          action: PayloadAction<(UpdateAppointmentPayload & { appointmentId: string }) | null>
        ) => {
      ...
      .addCase(
        cancelAppointmentAction.fulfilled,
        (state, action: PayloadAction<{ appointmentId: string } | null>) => {
          let updatedEvents = [...state.events?.events];
          updatedEvents = updatedEvents.map(event => {
            if (event.id === action.payload?.appointmentId) {
      ...
      .addCase(
        confirmAppointmentAction.fulfilled,
        (state, action: PayloadAction<{ appointmentId: string } | null>) => {
          let updatedEvents = [...state.events?.events];
          updatedEvents = updatedEvents.map(event => {
            if (event.id === action.payload?.appointmentId) {
      ...
      .addCase(editAppointmentRecurringAction.pending, state => {
        console.log('pending create appointment');
        state.state.loading = true;
      })
      .addCase(editAppointmentRecurringAction.fulfilled, state => {
        state.state = { ...state.state, success: true, loading: false, error: null, message: '' };
      })
      .addCase(editAppointmentRecurringAction.rejected, state => {
        state = initialState;
      })
      ...
      .addCase(
        ...
        (state, action: PayloadAction<{ id: string } | null>) => {
          state.state = { ...state.state, success: true, loading: false, error: null, message: '' };
        }
      )
